...
description: Aceitamos consultas sobre parcerias, contratos e entregas.
lang: pt
translations:
  ko: /contact/
  en: /contact/en/
...
Recebemos consultas sobre parcerias, contratos e entregas. Envie o formulário abaixo e ele será encaminhado à nossa equipe.
...
<form id="contract-form" style="max-width:720px;" data-workers-endpoint="{{ site.mail_gateway.workers_endpoint | default: '' }}" data-endpoint-hint="/assets/mail-endpoint.txt">
	<input type="hidden" name="_subject" id="contract_subject" value="[Consulta de contato] Envio">
	<input type="hidden" name="Category" value="Contract">
	<input type="text" name="website" style="display:none" tabindex="-1" autocomplete="off">

	<label>Empresa/Organização
		<input type="text" name="Company" required placeholder="Digite o nome da empresa/organização">
	</label>
	...
	<label>E-mail
		<input type="email" name="Email" required placeholder="nome@example.com">
	</label>

	<label>Assunto
		<input type="text" name="Title" id="contract_title" required placeholder="Resuma em uma linha">
	</label>

	<label>Mensagem
		<textarea name="Message" rows="6" required placeholder="Digite os detalhes"></textarea>
	</label>
...
(function(){
	var msgs = {
		sending: 'Enviando...',
		success: 'Obrigado! Sua consulta foi enviada. Entraremos em contato em breve.',
		error: 'Falha ao enviar. Por favor, tente novamente mais tarde.',
		offline: 'Você está offline. Verifique sua conexão e tente novamente.',
		mailto: 'Enviar por aplicativo de e-mail'
	};
	try {
	...
	function updateSubject(){
		var t = document.getElementById('contract_title').value || 'Sem assunto';
		document.getElementById('contract_subject').value = '[Consulta de contato] ' + t;
	}
		...
		form.addEventListener('submit', function(e){
			...
			loadEndpoint().then(function(submitUrl){
				...
				try {
					...
					var subj = document.getElementById('contract_subject').value || '[Consulta de contato] Envio';
