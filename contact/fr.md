...
description: Nous acceptons les demandes de partenariat, de contrat et de livraison.
lang: fr
translations:
  ko: /contact/
  en: /contact/en/
...
Nous accueillons les demandes concernant les partenariats, contrats et livraisons. Soumettez le formulaire ci-dessous et il sera envoyé à notre équipe.
...
<form id="contract-form" style="max-width:720px;" data-workers-endpoint="{{ site.mail_gateway.workers_endpoint | default: '' }}" data-endpoint-hint="/assets/mail-endpoint.txt">
	<input type="hidden" name="_subject" id="contract_subject" value="[Demande de contact] Soumission">
	<input type="hidden" name="Category" value="Contract">
	<input type="text" name="website" style="display:none" tabindex="-1" autocomplete="off">

	<label>Entreprise/Organisation
		<input type="text" name="Company" required placeholder="Entrez le nom de l'entreprise/organisation">
	</label>
	...
	<label>Objet
		<input type="text" name="Title" id="contract_title" required placeholder="Résumez en une ligne">
	</label>

	<label>Message
		<textarea name="Message" rows="6" required placeholder="Entrez les détails"></textarea>
	</label>
...
(function(){
	var msgs = {
		...
		offline: 'Vous êtes hors ligne. Veuillez vérifier votre connexion et réessayer.',
		mailto: 'Envoyer via application e-mail'
	};
	try {
	...
	function updateSubject(){
		var t = document.getElementById('contract_title').value || 'Sans objet';
		document.getElementById('contract_subject').value = '[Demande de contact] ' + t;
	}
		...
		form.addEventListener('submit', function(e){
			...
			loadEndpoint().then(function(submitUrl){
				...
				try {
					...
					var subj = document.getElementById('contract_subject').value || '[Demande de contact] Soumission';
