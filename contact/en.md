...
description: We accept partnership, contract, and delivery inquiries.
lang: en
translations:
  ko: /contact/
  en: /contact/en/
...
We welcome inquiries regarding partnerships, contracts, and deliveries. Submit the form below and it will be sent to our team.
...
<form id="contract-form" style="max-width:720px;" data-workers-endpoint="{{ site.mail_gateway.workers_endpoint | default: '' }}" data-endpoint-hint="/assets/mail-endpoint.txt">
	<input type="hidden" name="_subject" id="contract_subject" value="[Contract Inquiry] Submission">
	<input type="hidden" name="Category" value="Contract">
	<input type="text" name="website" style="display:none" tabindex="-1" autocomplete="off">

	<label>Company/Organization
		<input type="text" name="Company" required placeholder="Enter company/organization name">
	</label>
	...
	<label>Subject
		<input type="text" name="Title" id="contract_title" required placeholder="Summarize in one line">
	</label>

	<label>Message
		<textarea name="Message" rows="6" required placeholder="Enter detailed information"></textarea>
	</label>
...
(function(){
	var msgs = {
		sending: 'Sending...',
		success: 'Thank you! Your inquiry has been sent. We will contact you soon.',
		error: 'Failed to send. Please try again later.',
		offline: 'You are offline. Please check your connection and try again.',
		mailto: 'Send via Email App'
	};
	try {
	...
	function updateSubject(){
		var t = document.getElementById('contract_title').value || 'No subject';
		document.getElementById('contract_subject').value = '[Contract Inquiry] ' + t;
	}
		...
		form.addEventListener('submit', function(e){
			...
			loadEndpoint().then(function(submitUrl){
				...
				try {
					...
					var subj = document.getElementById('contract_subject').value || '[Contract Inquiry] Submission';
