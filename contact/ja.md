...
description: パートナーシップ・契約・納品に関するお問い合わせを受け付けています。
lang: ja
translations:
  ko: /contact/
  en: /contact/en/
...
パートナーシップ、契約、納品に関するお問い合わせを歓迎します。以下のフォームを送信すると、担当者にメールが届きます。
...
<form id="contract-form" style="max-width:720px;" data-workers-endpoint="{{ site.mail_gateway.workers_endpoint | default: '' }}" data-endpoint-hint="/assets/mail-endpoint.txt">
	<input type="hidden" name="_subject" id="contract_subject" value="[お問い合わせ] 送信">
	<input type="hidden" name="Category" value="Contract">
	<input type="text" name="website" style="display:none" tabindex="-1" autocomplete="off">

	<label>会社/団体名
		<input type="text" name="Company" required placeholder="会社/団体名を入力してください">
	</label>

	<label>担当者名
		<input type="text" name="Name" required placeholder="お名前を入力してください">
	</label>
	...
	<label>件名
		<input type="text" name="Title" id="contract_title" required placeholder="一行で要約してください">
	</label>

	<label>内容
		<textarea name="Message" rows="6" required placeholder="詳細を入力してください"></textarea>
	</label>
...
(function(){
	var msgs = {
		sending: '送信中...',
		success: 'ありがとうございます！お問い合わせが送信されました。まもなくご連絡いたします。',
		error: '送信に失敗しました。しばらくしてからもう一度お試しください。',
		offline: 'オフラインです。接続を確認してもう一度お試しください。',
		mailto: 'メールアプリで送信'
	};
	try {
	...
	function updateSubject(){
		var t = document.getElementById('contract_title').value || '件名なし';
		document.getElementById('contract_subject').value = '[お問い合わせ] ' + t;
	}
		...
		form.addEventListener('submit', function(e){
			...
			loadEndpoint().then(function(submitUrl){
				...
				if (status) {
					...
					status.textContent = msgs.error + (code ? ' (コード ' + code + ')' : '');
				}
				try {
					var mailtoBox = document.getElementById('contract-mailto');
					var to = 'captain@goolzy.com';
					var subj = document.getElementById('contract_subject').value || '[お問い合わせ] 送信';
