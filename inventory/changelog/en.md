...
The default prices for Stamps and Name Tag Badges have been reduced.
- **Stamp**: $9.99 → $0.99
...
Badge icons for users wearing badges are now displayed correctly in the log screen.
- **Badge Icon Display**: Badge images are displayed in comments written by users who are wearing badges
...
#### Quick Reply from Push Notifications
Reply directly from push notifications without opening the app.
- **Inline Reply**: Long-press a notification to type a reply that is instantly posted as a comment on the item
- **Offline Support**: When the network is unstable, replies are automatically queued and resent when the app launches
- **Multi-language Support**: Reply input hints are displayed according to the user's language settings
...
#### Badge Image Loading Improvements
Fixed the issue where badge images flickered on app restart.
- **Persistent Cache**: Badge image URLs are stored locally and persist across app restarts
- **Preloading**: All cached badge images are preloaded during splash screen
- **Background Refresh**: Badge images automatically update in the background when changed on the server
...
You can adjust the app's font size in 3 levels from the Settings screen.
- **3 Levels**: Choose from Default (1.0x), Medium (1.3x), or Large (1.5x)
- **Instant Apply**: Changes are applied immediately across the entire app
- **Persistent Settings**: Your preference is saved even after restarting the app

#### Auto Keyboard Dismiss
Tapping outside the log input field in item detail view now automatically dismisses the keyboard.
- **Tap to Dismiss**: Touch any area outside the input field to close the keyboard
- **iOS Support**: Conveniently use on iPhone without needing a dismiss button

#### Background Music Preservation
Music playing on your device (Spotify, YouTube Music, Apple Music, etc.) continues uninterrupted when you open the app.
- **Continuous Playback**: Background music from other apps keeps playing even when you launch the app
