---
layout: inventory
title: Inventory
permalink: /inventory/fr/
description: Goolzy Inventory - Application de gestion d'articles numériques
lang: fr
translations:
  ko: /inventory/
  en: /inventory/en/
  ja: /inventory/ja/
  de: /inventory/de/
  fr: /inventory/fr/
  es: /inventory/es/
  pt: /inventory/pt/
---

## 🎯 Inventory

**Créez des articles numériques et envoyez-les à d'autres !**

Créez des articles avec une seule feuille de papier et transférez-les via code QR. Gravez un nom de domaine unique avec un badge et créez des modèles avec des tampons à partager.

---

### 📚 Guide de démarrage rapide

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin: 2rem 0;">

  <div style="background: rgba(168, 85, 247, 0.2); backdrop-filter: blur(10px); border: 1px solid rgba(168, 85, 247, 0.3); padding: 1.5rem; border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.3);">
    <h3 style="margin: 0 0 .5rem 0; color: #a855f7; font-size: 1.3rem;">📋 Journal des modifications</h3>
    <p style="margin: 0 0 1rem 0; color: rgba(255,255,255,0.8); font-size: .95rem;">Consultez les dernières mises à jour</p>
    <a href="/inventory/changelog/fr/" style="display: inline-block; background: rgba(168,85,247,0.3); border: 1px solid rgba(168,85,247,0.5); color: white; padding: .5rem 1rem; border-radius: 8px; text-decoration: none; font-weight: 500; transition: all 0.3s;">Voir les détails →</a>
  </div>

  <div style="background: rgba(217, 70, 239, 0.2); backdrop-filter: blur(10px); border: 1px solid rgba(217, 70, 239, 0.3); padding: 1.5rem; border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.3);">
    <h3 style="margin: 0 0 .5rem 0; color: #d946ef; font-size: 1.3rem;">📖 Manuel</h3>
    <p style="margin: 0 0 1rem 0; color: rgba(255,255,255,0.8); font-size: .95rem;">Apprenez à utiliser l'app étape par étape</p>
    <a href="/inventory/manual/fr/" style="display: inline-block; background: rgba(217,70,239,0.3); border: 1px solid rgba(217,70,239,0.5); color: white; padding: .5rem 1rem; border-radius: 8px; text-decoration: none; font-weight: 500; transition: all 0.3s;">Commencer →</a>
  </div>

  <div style="background: rgba(20, 184, 166, 0.2); backdrop-filter: blur(10px); border: 1px solid rgba(20, 184, 166, 0.3); padding: 1.5rem; border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.3);">
    <h3 style="margin: 0 0 .5rem 0; color: #2dd4bf; font-size: 1.3rem;">❓ FAQ</h3>
    <p style="margin: 0 0 1rem 0; color: rgba(255,255,255,0.8); font-size: .95rem;">Questions fréquemment posées</p>
    <a href="/inventory/faq/fr/" style="display: inline-block; background: rgba(20,184,166,0.3); border: 1px solid rgba(20,184,166,0.5); color: white; padding: .5rem 1rem; border-radius: 8px; text-decoration: none; font-weight: 500; transition: all 0.3s;">Voir la FAQ →</a>
  </div>

</div>

---

### ✨ Fonctionnalités principales

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem; margin: 1.5rem 0;">

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; border-left: 4px solid #a855f7;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">📄</div>
    <h4 style="margin: 0 0 .5rem 0; color: #a855f7;">Création d'articles</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Créez des articles avec 1 feuille. Ajoutez titre, contenu, images et définissez dates de début/expiration.</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; border-left: 4px solid #14b8a6;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">🏷️</div>
    <h4 style="margin: 0 0 .5rem 0; color: #2dd4bf;">Système de badges</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Gravez un nom de domaine unique comme identité in-app. Votre domaine apparaît sur toutes les activités.</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; border-left: 4px solid #d946ef;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">🔖</div>
    <h4 style="margin: 0 0 .5rem 0; color: #d946ef;">Tampons & Journaux</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Créez des modèles avec des tampons. Laissez des entrées en mode privé ou partagé (communauté).</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; border-left: 4px solid #a855f7;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">📤</div>
    <h4 style="margin: 0 0 .5rem 0; color: #a855f7;">Transfert par code QR</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Scanner le QR du destinataire → Sélectionner l'article → Transfert terminé ! Propriété transférée instantanément.</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; border-left: 4px solid #14b8a6;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">🎨</div>
    <h4 style="margin: 0 0 .5rem 0; color: #2dd4bf;">Thèmes d'encre</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Changez les couleurs du thème avec de l'encre. Thèmes noir et or disponibles.</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; border-left: 4px solid #d946ef;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">🔍</div>
    <h4 style="margin: 0 0 .5rem 0; color: #d946ef;">Recherche en temps réel</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Filtrage instantané à la saisie. Enregistrez comme filtre pour une réutilisation rapide.</p>
  </div>

</div>

---

### 📝 Système de papier

<div style="background: rgba(168, 85, 247, 0.15); backdrop-filter: blur(10px); border: 1px solid rgba(168, 85, 247, 0.3); border-radius: 12px; padding: 1.5rem; margin: 1.5rem 0;">
  <p style="margin: 0 0 1rem 0; color: rgba(255,255,255,0.9); font-size: 1.05rem;"><strong>Papier</strong> est une monnaie in-app utilisée pour créer des documents.</p>
  <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 1rem;">
    <div style="background: rgba(10, 6, 20, 0.4); padding: 1rem; border-radius: 8px;">
      <div style="font-size: 1.5rem; margin-bottom: .25rem;">🎁</div>
      <p style="margin: 0; color: rgba(255,255,255,0.8); font-size: .9rem;"><strong>Nouvelle inscription</strong><br>10 feuilles gratuites</p>
    </div>
    <div style="background: rgba(10, 6, 20, 0.4); padding: 1rem; border-radius: 8px;">
      <div style="font-size: 1.5rem; margin-bottom: .25rem;">🔗</div>
      <p style="margin: 0; color: rgba(255,255,255,0.8); font-size: .9rem;"><strong>Lien social</strong><br>Google +10, Apple +10</p>
    </div>
    <div style="background: rgba(10, 6, 20, 0.4); padding: 1rem; border-radius: 8px;">
      <div style="font-size: 1.5rem; margin-bottom: .25rem;">💰</div>
      <p style="margin: 0; color: rgba(255,255,255,0.8); font-size: .9rem;"><strong>Achat en boutique</strong><br>Achat in-app</p>
    </div>
  </div>
  <p style="margin: 1rem 0 0 0; color: rgba(255,255,255,0.7); font-size: .85rem;">💡 Liez tous les comptes sociaux pour obtenir jusqu'à <strong>30 feuilles</strong> gratuitement !</p>
</div>

---

### 🛒 Articles de la boutique

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin: 1.5rem 0;">

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; text-align: center;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">📄</div>
    <h4 style="margin: 0 0 .25rem 0; color: #a855f7;">Papier</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Pour création de documents</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; text-align: center;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">🔖</div>
    <h4 style="margin: 0 0 .25rem 0; color: #14b8a6;">Tampon</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Pour création de modèles</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; text-align: center;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">🏷️</div>
    <h4 style="margin: 0 0 .25rem 0; color: #d946ef;">Badge</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Pour gravure de domaine</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; text-align: center;">
    <div style="font-size: 2rem; margin-bottom: .5rem;">🎨</div>
    <h4 style="margin: 0 0 .25rem 0; color: #a855f7;">Encre</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">Pour changement de couleur</p>
  </div>

</div>

---

### 📱 Plateformes prises en charge

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin: 1.5rem 0;">

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; text-align: center;">
    <div style="font-size: 2.5rem; margin-bottom: .5rem;">📱</div>
    <h4 style="margin: 0 0 .25rem 0; color: #a855f7;">Mobile</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">iOS / Android</p>
  </div>

  <div style="padding: 1.25rem; background: rgba(10, 6, 20, 0.4); backdrop-filter: blur(10px); border-radius: 12px; text-align: center;">
    <div style="font-size: 2.5rem; margin-bottom: .5rem;">💻</div>
    <h4 style="margin: 0 0 .25rem 0; color: #14b8a6;">Bureau</h4>
    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: .9rem;">macOS / Windows</p>
  </div>

</div>

<p style="text-align: center; color: rgba(255,255,255,0.6); font-size: .9rem; margin-top: 1rem;">Mêmes fonctionnalités sur toutes les plateformes</p>

---

### 🌍 Support multilingue

7 langues prises en charge :
- 🇰🇷 한국어, 🇺🇸 English, 🇯🇵 日本語
- 🇩🇪 Deutsch, 🇫🇷 Français, 🇪🇸 Español, 🇧🇷 Português

---

### 🆘 Besoin d'aide ?

<div style="background: rgba(20, 184, 166, 0.15); backdrop-filter: blur(10px); border: 1px solid rgba(20, 184, 166, 0.3); border-radius: 12px; padding: 1.5rem; margin: 1.5rem 0;">
  <p style="margin: 0 0 1rem 0; color: rgba(255,255,255,0.9);">Des problèmes ou voulez-vous suggérer une nouvelle fonctionnalité ?</p>
  <div style="display: flex; gap: .75rem; flex-wrap: wrap;">
    <a href="/inventory/suggest/fr/" style="display: inline-block; background: rgba(168,85,247,0.3); border: 1px solid rgba(168,85,247,0.5); color: white; padding: .6rem 1.2rem; border-radius: 8px; text-decoration: none; font-weight: 500; transition: all 0.3s;">💬 Suggérer</a>
    <a href="/inventory/bug-report/fr/" style="display: inline-block; background: rgba(217,70,239,0.3); border: 1px solid rgba(217,70,239,0.5); color: white; padding: .6rem 1.2rem; border-radius: 8px; text-decoration: none; font-weight: 500; transition: all 0.3s;">🐛 Signaler un bug</a>
  </div>
</div>

---

> 🔒 **Protection de la vie privée** : Vos données sont cryptées et protégées. Voir notre [Politique de confidentialité](/inventory/privacy/) pour plus de détails.

---

{% include usecases-scroll.html lang="fr" %}

<style>
.neumorphic-content h2 {
  background: linear-gradient(135deg, #a855f7, #2dd4bf) !important;
  background-clip: text !important;
  -webkit-background-clip: text !important;
  -webkit-text-fill-color: transparent !important;
  border-bottom: 2px solid rgba(168, 85, 247, 0.3) !important;
  padding-bottom: .5rem !important;
  margin-bottom: 1.5rem !important;
}

.neumorphic-content h3 {
  color: #a855f7 !important;
  margin-top: 2.5rem !important;
  margin-bottom: 1rem !important;
}

.neumorphic-content h4 {
  font-size: 1.05rem !important;
}

.neumorphic-content hr {
  border: none !important;
  height: 1px !important;
  background: linear-gradient(90deg, transparent, rgba(168, 85, 247, 0.3), transparent) !important;
  margin: 2.5rem 0 !important;
}

.neumorphic-content blockquote {
  background: rgba(20, 184, 166, 0.15) !important;
  backdrop-filter: blur(10px) !important;
  border-left: 4px solid #2dd4bf !important;
  padding: 1rem 1.5rem !important;
  margin: 1.5rem 0 !important;
  border-radius: 12px !important;
  color: rgba(255,255,255,0.9) !important;
}

@media (max-width: 768px) {
  div[style*="grid-template-columns"] { grid-template-columns: 1fr !important; }
}
</style>
