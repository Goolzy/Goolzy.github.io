...
#### 글래스모피즘 + 드롭렛 디자인
앱 전체가 글래스모피즘과 드롭렛 스타일로 전면 개편되었습니다.
...
- 보내기, 삭제, 잠금/해제를 인라인 UI로 바로 실행

#### 로그 화면 개편
로그가 폴더별로 그룹화되어 표시됩니다.
- 폴더 단위로 정리된 로그 목록과 새로운 애니메이션
- 기존 단독 로그 시스템이 공유 로그로 통합

#### 검색 및 필터 개선
- 공유 태그 검색 필드가 헤더로 이동하여 접근성 향상
...
#### 제거된 기능
- 클립(묶기) 기능이 제거되었습니다
- 하단 네비게이션 바의 위자드 메뉴가 제거되었습니다

#### 버그 수정
- QR 스캐너 카메라 초기화 시 발생하던 멈춤 현상 수정
- 폴더 이름 인라인 편집 시 스크롤 및 포커스 문제 수정
- 로그 스크롤 시 애니메이션 오발동 수정
- 시스템 로그가 불필요한 알림을 발생시키던 문제 수정
...
#### 키워드 동기화 강화
키워드를 삭제하거나 변경하면 스탬프, 공유 템플릿, 복제 아이템에 자동 반영됩니다.

#### 그리드 뷰 개선
화면 크기에 따라 열 수가 자동 계산되며, 아이템 텍스트가 항상 표시됩니다.

#### 상세 검색 헤더 개선
아이템 상세 검색 화면의 헤더 UI가 개선되었습니다.

#### 하단 네비게이션 탭 순서 변경
하단 네비게이션 탭 순서가 재구성되었습니다.

#### 로그 탭 초기 안내
로그가 없는 초기 사용자에게 안내 화면이 표시됩니다.
...
- 공유 설정 다이얼로그 라벨 색상 테마 미적용 수정
- Windows Store 인증 실패 문제 수정
- 흔들림 효과가 모든 아이템에 적용되던 문제 수정
