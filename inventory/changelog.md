...
- **드래그로 확장**: 핸들바를 아래로 드래그하거나 사용자명을 탭하여 프로필 패널 확장
- **통합 패널**: 헤더와 프로필 콘텐츠(프로필 카드, QR 코드, API 정보)가 하나의 패널로 통합
- **부드러운 애니메이션**: 확장/축소 시 자연스러운 애니메이션 효과
- **고정된 메인 콘텐츠**: 프로필 확장 시에도 인벤토리 그리드는 제자리 유지

#### 포켓 리브랜딩
아이템을 분류하는 단위의 이름이 '파티션'에서 '포켓'으로 변경되었습니다.
- **직관적인 이름**: 물건을 담는 주머니처럼 아이템을 담는 공간이라는 의미를 더 잘 전달
- **일관된 UI**: 앱 전체에서 '포켓'이라는 용어로 통일
- **다국어 지원**: 한국어(포켓), 영어(Pocket), 일본어(ポケット), 중국어(口袋) 등 각 언어에 맞게 번역

#### 기존 데이터 자동 마이그레이션
기존 '파티션' 데이터가 '포켓'으로 자동 변환됩니다.
- **무손실 마이그레이션**: 기존에 만들어둔 파티션과 아이템이 그대로 유지
- **자동 처리**: 앱 업데이트 시 자동으로 마이그레이션 진행
- **투명한 전환**: 사용자가 별도로 조치할 필요 없이 바로 사용 가능

#### 기본 포켓
첫 번째 포켓이 헤더 없이 기본 포켓으로 동작합니다.
- **헤더 숨김**: 기본 포켓은 포켓 헤더가 표시되지 않아 더 깔끔한 UI
- **항상 최상단**: 기본 포켓은 항상 인벤토리 최상단에 위치
- **접기 불가**: 기본 포켓은 항상 펼쳐진 상태로 유지
- **자동 아이템 수신**: 새로 구입하거나 받은 아이템은 기본 포켓으로 자동 이동

#### 액션 스피드 다이얼 FAB
아이템 선택 시 사용 가능한 액션들이 플로팅 버튼으로 표시됩니다.
- **플로팅 액션 버튼**: 아이템 선택 시 우측 하단에 번개 모양 FAB 표시
- **스피드 다이얼**: FAB 탭 시 위로 확장되어 가능한 액션 버튼들 표시
- **액션 갯수 배지**: FAB에 현재 사용 가능한 액션 수가 배지로 표시
- **부드러운 애니메이션**: FAB 나타나기/사라지기 스케일 애니메이션, 액션 갯수 변경 시 배지 팡팡 효과

#### 아이템 선택 UI 개선
선택한 아이템의 시각적 피드백이 개선되었습니다.
- **바깥쪽 아웃라인**: 선택 테두리가 이미지 바깥에 표시되어 원본 크기 유지
- **메인컬러 테두리**: 선택 아웃라인이 테마 메인컬러로 통일
- **겹치는 테두리**: 인접한 아이템 선택 시 테두리가 서로 겹쳐 선택 영역을 명확하게 표시
- **흔들림 효과 제거**: 선택된 아이템의 흔들림 효과가 제거되어 더 깔끔한 UI

#### 신규 로그 알림 개선
신규 로그가 있는 아이템의 알림 방식이 변경되었습니다.
- **흔들림 효과**: 점멸하는 테두리 대신 미세한 흔들림으로 신규 로그 표시
- **직관적 인식**: 선택 아웃라인과 구분되어 알림을 더 쉽게 인식

#### 우편함 바로가기
하단 네비게이션 바에서 우편함(받기 화면)으로 바로 이동할 수 있습니다.
- **빠른 접근**: 홈 화면에서 바로 받은 아이템 확인 가능
- **직관적인 아이콘**: 편지 모양 아이콘으로 우편함 기능 표현

#### 상점 화면 UI 간소화
상점 화면의 헤더 영역이 간결하게 정리되었습니다.
- **중복 타이틀 제거**: Shell 모드에서 네비바 타이틀과 중복되던 '상점' 제목 삭제
- **설명 문구 제거**: 불필요한 설명 문구를 삭제하여 상품 목록에 집중
- **공지 상단 배치**: Firebase에서 가져오는 공지 메시지가 탭바 위 최상단에 표시

#### 인벤토리 홈 오프닝 애니메이션
인벤토리 홈 화면에도 다른 탭과 동일한 오프닝 애니메이션이 적용됩니다.
- **헤더 슬라이드 인**: 앱 시작 시 프로필 헤더가 위에서 부드럽게 내려옴
- **탭 전환 애니메이션**: 다른 탭에서 홈으로 돌아올 때도 동일한 애니메이션 적용
- **일관된 UX**: 모든 탭에서 동일한 오프닝 연출로 통일감 있는 사용자 경험

#### 카테고리 필터 바
인벤토리 최상단에서 카테고리별 필터와 텍스트 필터를 사용할 수 있습니다.
- **Pull-to-reveal**: 맨 위에서 아래로 당겨 필터 바 표시, 스크롤 시 자동 숨김
- **카테고리 토글**: 전체/아이템만/재료만/문서만 순환 필터링
- **텍스트 필터 추가**: + 버튼으로 사용자 정의 필터 추가 (OR 조건)
- **뉴모픽 디자인**: 앱 테마와 일관된 뉴모픽 스타일 버튼/칩
- **다국어 지원**: 8개 언어에서 카테고리 라벨과 힌트 텍스트 지원

#### 설정 화면 개편
설정 화면이 5개의 접이식 그룹으로 재구성되었습니다.
- **5개 그룹**: 색상, 글꼴, 소리, 푸쉬알림, 시스템으로 설정 항목 분류
- **접기/펼치기**: 각 그룹을 탭하여 접거나 펼칠 수 있는 아코디언 UI
- **푸쉬알림 간소화**: 7개 카테고리가 3개(거래, 로그, 만료)로 통합
- **키보드 접근성**: 방향키로 그룹 접기/펼치기, 항목 이동 지원

#### 버그 수정
- **알림창 표시 수정**: 프로필 헤더의 알림 패널에서 알림이 올바르게 표시됩니다.
  - 알림 타입별 아이콘과 색상이 정확하게 표시됩니다
  - 알림 제목과 내용이 다국어로 올바르게 표시됩니다
  - 알림 아이템의 둥근 모서리가 메뉴 버튼과 동일하게 통일되었습니다
...
- **태그 테마 컬러**: 인기/최신 태그가 고정 색상 대신 앱 테마 색상으로 표시
...
#### 로그 네비게이션 통합
로그에서 클립 아이템을 탭하면 그리드와 동일하게 클립 상세 화면으로 진입합니다.
- **클립 상세 화면 진입**: 로그에서 클립 아이템 탭 시 클립 내 모든 페이지를 스와이프할 수 있는 클립 상세 화면으로 이동
- **정확한 페이지 위치**: 탭한 아이템의 클립 내 순서에 해당하는 페이지에서 시작
- **통합된 경로**: 그리드, 로그, 키보드 네비게이션 모두 동일한 상세 화면 경로 사용
