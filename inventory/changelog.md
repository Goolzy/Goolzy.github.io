...
#### 파티션 관리 기능 개선
뉴모픽 디자인 파티션 이름 설정 다이얼로그와 드래그 순서 변경 기능이 추가되었습니다.
- **뉴모픽 다이얼로그**: 파티션 이름 설정에 새로운 디자인 적용 (다크/라이트 모드 지원)
- **드래그 순서 변경**: ≡ 핸들을 길게 눌러 파티션 순서를 드래그로 변경
- **생성 시 이름 입력**: 새 파티션 생성 시 바로 이름 설정 가능
