...
#### 로그 화면 미읽음 알림 개선
로그 목록에서 미읽음 상태를 더 명확하게 확인할 수 있습니다.
- **미읽음 카운트 뱃지**: 읽지 않은 댓글 수가 숫자 뱃지로 표시 (99+까지)
- **읽음/미읽음 색상 구분**: 미읽음 로그는 밝은 배경, 읽은 로그는 어두운 배경으로 시각적 구분
- **실시간 업데이트**: 새 댓글이 작성되면 실시간으로 카운트 증가, 확인 후 자동 리셋
...
- **포켓 접기/펼치기 애니메이션 개선**: 포켓을 접을 때 콘텐츠가 순간적으로 사라지던 문제가 수정되어 부드럽게 위로 접히는 애니메이션으로 개선되었습니다.
  - 접기: 콘텐츠가 아래에서 위로 자연스럽게 접힘
  - 펼치기: 콘텐츠가 위에서 아래로 부드럽게 펼쳐짐
...
#### 그리드 아이템 카드 디자인 개선
인벤토리 그리드의 아이템 카드가 더 깔끔한 플랫 디자인으로 변경되었습니다.
- **그림자 제거**: 밀집된 그리드에서 inset 그림자처럼 보이던 BoxShadow 완전 제거
- **하단 그라디언트 제거**: 카드 아래쪽의 어두운 그라디언트 오버레이 제거
- **플랫 디자인**: 깔끔하고 현대적인 플랫 스타일로 통일
- **텍스트 레이블 유지**: 아이템 타입 레이블은 흰색 텍스트 + 그림자로 가독성 확보

#### 위자드 채팅방 생성 개선
채팅방 위자드가 더욱 직관적으로 개선되었습니다.
- **그룹 대화 인원 표시**: 기본 제목이 'n인 그룹 대화'로 표시되어 참여자 수를 바로 확인
- **완성 이미지 미리보기**: 위자드 메뉴에 아이콘 대신 실제 완성될 아이템 이미지가 표시
- **빠른 이미지 로딩**: 로컬 이미지 사용으로 매번 다운로드 없이 즉시 표시

#### 1:1 채팅방 동적 제목
1:1 채팅방에서 보낸 사람과 받는 사람이 각자 상대방 이름으로 제목을 볼 수 있습니다.
- **상대방 이름 표시**: 보낸 사람은 "받는사람님과의 대화", 받는 사람은 "보낸사람님과의 대화"로 표시
- **커스텀 제목 우선**: 직접 입력한 제목은 양쪽 모두에게 동일하게 표시
- **그룹 채팅 동일 제목**: 그룹 채팅은 모든 참여자에게 동일한 제목 표시
